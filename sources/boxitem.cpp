    ...
    _imageSize(imageSize)
{
    ...
    _textName.setDefaultTextColor(QColor(153,0,51,255));
    ...
    case Qt::LeftButton:
        if (this->isSelected()) {
            ...
            if (_selectedGrabber != BoxRegion) { // grabber is selected
                _taskStatus = TaskStatus::Stretching;
            } else if (_rect.contains(event->pos())) { // box is selected
                _taskStatus = TaskStatus::Moving;
//                QMessageBox::critical(0 , "Alert" , "包含该点", QMessageBox::Ok | QMessageBox::Default , QMessageBox::Cancel | QMessageBox::Escape ,0 );
            }
...
void BoxItem::paint (QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if (option->state & QStyle::State_Selected) {
        ...
        painter->fillRect(rect(),QColor(0,255,57,57));//填充颜色,其中第一属性为透明度
        emit boxSelected(getRealRect(), _typeName);
    }

    _textName.setPos(_rect.topLeft());//设置位置
    _textName.setPlainText(_typeName);
    _textName.setScale(0.9);
}
...
void BoxItem::setGrabbers(qreal width, qreal height)
{
    ...
    _grabbers[TopLeft     ].setRect(_rect.left()-w/2,     _rect.top()-h/2,       w, h);
    _grabbers[TopLeft     ].setRect(_rect.left()-w/2,     _rect.top()-h/2,       w, h);
//    _grabbers[TopLeft     ].setRect(_rect.left()-w,     _rect.top()-h,       width, height);
    _grabbers[TopRight    ].setRect(_rect.right()-w/2,    _rect.top()-h/2,       w, h);
    _grabbers[BottomLeft  ].setRect(_rect.left()-w/2,     _rect.bottom()-h/2,    w, h);
    _grabbers[BottomRight ].setRect(_rect.right()-w/2,    _rect.bottom()-h/2,    w, h);
...
//    _grabbers[TopCenter   ].setRect(_rect.center().x()-w,   _rect.top()-h,           width, height);//设置显示角点
...
{
    qDebug()<<"now contextMenuEvent is run...........";
